...
def validate_meta(path: Path, meta: dict[str, str], site_dir: Path) -> dict[str, object]:
    ...
    if language not in {"zh-CN", "en"}:
        raise PublishError(f"{path}: language must be zh-CN or en")
...
def article_page(
    ...
    zh_post: dict[str, object], zh_body: str, zh_minutes: int,
) -> str:
    ...
    zh_author = html.escape(str(zh_post["author"]))
    slug = str(post["slug"])
    date = post["date_obj"]
    assert isinstance(date, dt.date)
    canonical = f"{SITE_URL}/blog/{slug}/"
    date_label = date.strftime("%d %b %Y")
    zh_date_label = f"{date.year}年{date.month}月{date.day}日"
    read_label = f"{minutes} min read"
    zh_read_label = f"约 {zh_minutes} 分钟阅读"
    schema = json.dumps({
        ...
        "inLanguage": ["en", "zh-CN"],
        "alternativeHeadline": str(zh_post["title"]),
...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}}body{{margin:0;color:var(--md-ink);background:#f5f6f7;font-family:Arial,"Noto Sans SC",sans-serif;line-height:1.75}}
    [data-lang-copy="zh-CN"],[data-lang-panel="zh-CN"]{{display:none}}html[lang="zh-CN"] [data-lang-copy="en"],html[lang="zh-CN"] [data-lang-panel="en"]{{display:none}}html[lang="zh-CN"] [data-lang-copy="zh-CN"]{{display:inline}}html[lang="zh-CN"] [data-lang-panel="zh-CN"]{{display:block}}
    ...
    .md-top-inner{{padding:8px 0;display:flex;justify-content:flex-end;align-items:center;gap:18px}}.md-lang{{display:flex;border:1px solid rgba(255,255,255,.35)}}.md-lang button{{border:0;background:transparent;color:#fff;padding:5px 9px;cursor:pointer}}.md-lang button[aria-pressed="true"]{{background:var(--md-orange);color:#111}}.md-nav{{background:#111a25;color:#fff;border-bottom:1px solid rgba(255,255,255,.12)}}
...
<body>
  <div class="md-top"><div class="md-top-inner"><div class="md-lang" role="group" aria-label="Language"><button type="button" data-set-language="en">English</button><button type="button" data-set-language="zh-CN">中文</button></div><span>{dual('Call us','致电')} +86 186 2024 4613</span></div></div>
  <header class="md-nav"><div class="md-nav-inner">
    ...
    <nav class="md-links" aria-label="Main navigation"><a href="/about/">{dual('About Us','关于我们')}</a><a href="/services/">{dual('Our Services','服务项目')}</a><a href="/blog/">{dual('Blog','博客')}</a><a href="/contact/">{dual('Contact Us','联系我们')}</a><a class="md-action" href="https://wa.me/8618620244613">{dual('Speak to an expert →','咨询物流顾问 →')}</a></nav>
  </div></header>
  <main>
    <section class="md-hero" style="background-image:url('{cover}')"><div class="md-hero-inner">
      <div class="md-kicker">{dual(category,zh_category)}</div><h1>{dual(title,zh_title)}</h1><p class="md-summary">{dual(summary,zh_summary)}</p>
      <div class="md-meta"><span>{dual(date_label,zh_date_label)}</span><span>{dual(read_label,zh_read_label)}</span><span>{dual(author,zh_author)}</span></div>
    </div></section>
    <div class="md-wrap"><article class="md-content"><a class="md-back" href="/blog/">← {dual('Back to Blog','返回博客')}</a><section data-lang-panel="en">{body}</section><section data-lang-panel="zh-CN">{zh_body}</section></article>
      <aside class="md-side"><h2>{dual('Planning a shipment from China?','准备从中国安排运输？')}</h2><p>{dual('Send us the cargo details and destination. We will help plan the China-side process.','把货物信息和目的地发给我们，我们会协助规划中国端运输流程。')}</p><a href="https://wa.me/8618620244613">{dual('Speak to Vinson','联系 Vinson')} →</a></aside>
    </div>
  </main>
...
def blog_section(posts: list[dict[str, object]]) -> str:
    ...
    for post in posts:
        zh_post = post["zh"]
        assert isinstance(zh_post, dict)
        date = post["date_obj"]
        assert isinstance(date, dt.date)
        date_label = date.strftime("%d %b %Y")
        zh_date_label = f"{date.year}年{date.month}月{date.day}日"
        read_label = f"{post['minutes']} min read"
        zh_read_label = f"约 {zh_post['minutes']} 分钟阅读"
...
def publish(site_dir: Path, articles_dir: Path, translations_dir: Path) -> int:
    ...
    for path in files:
        ...
        if destination.exists():
            raise PublishError(f"{path}: route already exists in the stable website: /blog/{post['slug']}/")
        ...
        if zh_post["date_obj"] != post["date_obj"]:
            raise PublishError(f"{translation_path}: translation date must match English article")
        zh_body, zh_minutes = markdown_to_html(zh_source)
        zh_post["body"] = zh_body
        zh_post["minutes"] = zh_minutes
        post["zh"] = zh_post
        posts.append(post)
    ...
    for post in posts:
        ...
        (destination / "index.html").write_text(
            article_page(
                post, str(post["body"]), int(post["minutes"]),
                zh_post, str(zh_post["body"]), int(zh_post["minutes"]),
            ),
            encoding="utf-8",
        )
...
def main() -> int:
    ...
    if not (site_dir / "index.html").is_file() or not (site_dir / "blog" / "index.html").is_file():
        ...
        return 2
    try:
        count = publish(site_dir, args.articles.resolve(), args.translations.resolve())
